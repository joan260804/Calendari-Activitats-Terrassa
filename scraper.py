...
def obtener_eventos_tae():
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"[TAE] Error {resp.status_code} al acceder a {URL_TAE}")
            return eventos
        
        soup = BeautifulSoup(resp.text, "html.parser")
        # Buscar elementos que contienen eventos en la web de Terrassa Arts Escèniques
        ...
        for card in cards:
            ...
            if title_el and link_el:
                ...
                # Parseo básico de fechas ISO (YYYY-MM-DD)
                fecha_iso = extraer_fecha_iso(fecha_raw)
                dia_nom = extraer_nombre_dia(fecha_raw)
                hora_str = extraer_hora(fecha_raw)
        ...
        print(f"[TAE] Excepción durante el scraping: {e}")
...
def obtener_eventos_lafact():
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"[LaFACT] Error {resp.status_code} al acceder a {URL_LAFACT}")
            return eventos
        ...
        print(f"[LaFACT] Excepción durante el scraping: {e}")
...
def obtener_eventos_tnt():
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"[TNT] Error {resp.status_code} al acceder a {URL_TNT}")
            return eventos
        ...
        print(f"[TNT] Excepción durante el scraping: {e}")

    return eventos

# Funciones auxiliares de limpieza y extracción de datos
...
def actualizar_index():
    ...
    if not todos_eventos:
        print("No se encontraron nuevos eventos o la lista obtenida está vacía. No se modifica index.html.")
        return
